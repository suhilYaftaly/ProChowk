...
import { Button, Form, Input, TextArea, YStack } from 'tamagui';
...
const UserProfileSetup = () => {
  ...
  /* useEffect(() => {
    if (userTypeQuery) setForm((prev) => ({ ...prev, userType: userTypeQuery }));
  }, [userTypeQuery]); */
          ...
          onSuccess: (newUser) => {
            if (newUser.userTypes.length > 0 && !newUser.emailVerified) {
              ...
              if (user) {
                ...
                router.navigate({ pathname: '/dashboard', params: { username: username } });
              }
            }
          },
        });
      ...
      <Form onSubmit={onSubmit}>
        <YStack space={'$4'} marginTop={30}>
          <YStack space={'$1.5'}>
            ...
            />
            {!userTypeAvail && <Text style={{ color: 'red' }}>*{labels.userTypeError}</Text>}
          </YStack>
        ...
        <Form.Trigger asChild>
          <Button backgroundColor="$primary" style={styles.button}>
            Get Start
          </Button>
        </Form.Trigger>
      </Form>
