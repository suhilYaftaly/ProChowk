...
export const isSuperAdmin = (roles: Role[] | undefined) => roles?.includes('superAdmin') || false;
...
export const isDeveloper = (roles: Role[] | undefined): boolean =>
  roles?.includes('dev') || roles?.includes('admin') || roles?.includes('superAdmin') || false;
...
export const isUserClientAndContractor = (userTypes: UserType[] | undefined): boolean => {
  if (!userTypes) return false;
  return userTypes.includes('client') && userTypes.includes('contractor');
};

export const getLocalTokens = (): ITokens | undefined => getLocalData(TOKENS_KEY);
