...
const DELETE_CONTRACTOR_PORTFOLIO = gql`
  mutation DeleteContractorPortfolio($id: ID!) {
    deleteContractorPortfolio(id: $id)
  }
`;

type PortfolioImage = IImage;
...
export const updateContractorPortfoliosCache = ({
  ...
  switch (action) {
    case "add":
      if (portfolio) {
        updatedPortfolios = [
          ...existingPortfolios.getContractorPortfolios,
          portfolio,
        ];
      }
      break;
    case "update":
      if (portfolio) {
        updatedPortfolios = existingPortfolios.getContractorPortfolios.map(
          (p) => (p.id === portfolio.id ? portfolio : p)
        );
      }
      break;
    case "delete":
...
export const useContractorPortfolio = () => {
  const [getContractorPortfolios, { data, error, loading }] = useLazyQuery<
    GCPData,
    GCPInput
  >(GET_CONTRACTOR_PORTFOLIOS);

  const getContractorPortfoliosAsync = async ({
    variables,
    onSuccess,
    onError,
  }: GCPAsync) =>
    asyncOps({
      ...
      onSuccess: (dt: GCPData) =>
        onSuccess && onSuccess(dt.getContractorPortfolios),
      onError,
    });
...
export const useAddContractorPortfolio = () => {
  ...
  const [addContractorPortfolio, { data, error, loading }] = useMutation<
    ACPData,
    ACPInput
  >(ADD_CONTRACTOR_PORTFOLIO);

  const addContractorPortfolioAsync = async ({
    variables,
    onSuccess,
    onError,
  }: ACPAsync) =>
...
export const useUpdateContractorPortfolio = () => {
  const client = useApolloClient();
  const [updateContractorPortfolio, { data, error, loading }] = useMutation<
    UCPData,
    UCPInput
  >(UPDATE_CONTRACTOR_PORTFOLIO);

  const updateContractorPortfolioAsync = async ({
    variables,
    onSuccess,
    onError,
  }: UCPAsync) =>
...
export const useDeleteContractorPortfolio = () => {
  const client = useApolloClient();
  const [deleteContractorPortfolio, { data, error, loading }] = useMutation<
    any,
    DCPInput
  >(DELETE_CONTRACTOR_PORTFOLIO);

  const deleteContractorPortfolioAsync = async ({
    variables,
    onSuccess,
    onError,
  }: DCPAsync) =>
